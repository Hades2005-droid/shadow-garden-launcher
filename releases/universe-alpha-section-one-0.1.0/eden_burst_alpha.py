...
BLACK_SUN_STATE = ROOT / "black_sun_learning" / "state"
...
def load_black_sun_status() -> dict[str, Any]:
    path = BLACK_SUN_STATE / "latest.json"
    if path.exists():
        try:
            return json.loads(path.read_text(encoding="utf-8"))
        except Exception as exc:
    ...
    return {"status": "not_initialized", "symbolic_only": True}
...
class EdenHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/api/drake":
            ...
            return
        if path == "/api/black-sun":
            self._json(200, load_black_sun_status())
            return
        if path == "/healthz":
